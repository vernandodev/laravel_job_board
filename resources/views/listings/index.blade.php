    ...
    <section class="container px-5 py-12 mx-auto">
        ...
        <div class="mb-12">
            <h2 class="text-2xl font-medium text-gray-900 title-font px-4">
                All Jobs {{ $listings->count() }} <!-- get total semua job list -->
            </h2>
        </div>
        <div class="-my-6">
            @foreach($listings as $listing)
                <a href="{{ route('listings.show', $listing->slug) }}" class="py-6 px-4 flex flex-wrap md:flex-nowrap border-gray-100 {{ $listing->is_highlighted ? 'bg-yellow-100 hover:bg-yellow-200' : 'bg-white hover:bg-gray-100'}}">
